...
public class InstallDocMgtImpl implements InstallDocMgt {
	...
	public String loadInstallDocumentByEventCode(String argEventCodeText) {
		...
		if (StringUtils.isNotEmpty(userId)) {
			...
			currentUser = (RmsUser) session
					.getAttribute(WosConstant.CURRENT_USER);
		}

		EventInfo eventInfo = eventInfoMapper
				.loadEventInfoByEventCode(eventCode);
	...
	public String getAllInstallTemplates() {
		List<InstallTemplate> templates = installTemplateMapper
				.loadAllInstallTemplates();
	...
	public String getInstallDetailByTemplate(String argInstallTemplateText) {
		String installTemplateId = _helper.getValueFromJsonText(
				argInstallTemplateText, "installTemplateId");
		String installDocumentId = _helper.getValueFromJsonText(
				argInstallTemplateText, "installDocumentId");

		List<InstallDetail> installDetails = installDetailMapper
				.findInstallDetailByTemplateId(installTemplateId);

		List<InstallDocuDetail> installDocuDetails = createInstallDocDetailsFromInstallDetail(
				installDetails, installDocumentId);
	...
	public String getAllTaxOrganizations() {
		List<TaxOrganization> taxOrganizations = taxOrganizationMapper
				.loadAllTaxOrganizations();
	...
	public String getTaxOrganizationsByParentId(String argParentIdText) {
		String parentId = _helper.getValueFromJsonText(argParentIdText,
				"cparentid");
		List<TaxOrganization> taxOrganizations = taxOrganizationMapper
				.findTaxOrganizationsByParentCode(parentId);
		return _helper.toJsonText(taxOrganizations, null);
	}
	...
	public String getTaxOrganizationsByName(String argOrgNameText) {
		...
		List<TaxOrganization> taxOrganizations = taxOrganizationMapper
				.findTaxOrganizationByName(orgName);
	...
	public String getEnterpriseContactInfo(String argEnterpriseIdText) {
		...
		List<EnterpriseContacts> enterpriseContactRes = enterpriseContactMapper
				.findContactInfoByEnterpriseId(enterpriseId);
	...
	public String getContactInfoById(String argContactIdText) {
		String contactInfoId = _helper.getValueFromJsonText(argContactIdText,
				"cGUID");
		List<ContactInfo> contactInfos = contactInfoMapper
				.findContactInfoById(contactInfoId);
		...
		if (contactInfoRes != null) {
			...
			if (mapRes != null) {
				...
				if (mapRes.get("cisonjob") != null) {
					enterpriseContact.setCisonjob(Integer.valueOf(mapRes
							.get("cisonjob")));
				}
				if (mapRes.get("isnew") != null) {
					enterpriseContact.setIsnew(Short.valueOf(mapRes
							.get("isnew")));
				}
	...
	public String selectAsCurrentContact(String argCurrentContactText) {
		...
		EnterpriseContacts newEnterpriseContact = enterpriseContactMapper
				.findContactInfoByPrimaryId(cguid);
	...
	private void updateContactToInstallDoc(String newContactId,
			String enterpriseId) {
		List<ContactInfo> newContactInfos = contactInfoMapper
				.findContactInfoById(newContactId);
		...
		if (newContactInfos != null && newContactInfos.size() > 0) {
			...
			List<InstallDocument> documents = installDocumentMapper
					.findInstallDocumentByEnterpriseId(enterpriseId);
	...
	public String getEnterpriseAddresses(String argEnterpriseIdText) {
		...
		List<EnterpriseAddress> enterpriseAddresses = enterpriseAddressMapper
				.getEnterpriseAddresses(enterpriseId);
	...
	public String addEnterpriseAddress(String argAddressText) {
		...
		int result = enterpriseAddressMapper
				.insertSelective(enterpriseAddressNew);
		return _helper.toJsonText(result, null);
	}

	@Override
	public String updateEnterpriseAddress(String argAddressText) {
		EnterpriseAddress enterpriseAddressUpdate = _gson.fromJson(
				argAddressText, EnterpriseAddress.class);

		int result = enterpriseAddressMapper
				.updateEnterpriseAddress(enterpriseAddressUpdate);
		return _helper.toJsonText(result, null);
	}
	...
	public String loadAllServiceResponses() {
		List<ServiceResponse> serviceResponses = serviceResponseMapper
				.loadAllServiceResponses();
	...
	public String loadExtendedAttributesByMatId(String argMatIdText) {
		...
		List<ExtendedAttribute> extendedAttributes = extendedAttributeMapper
				.findExtendedAttributesByMatId(matId);
	...
	public String addInstallDocDetail(String installDetailText) {
		InstallDocuDetail installDocuDetailNew = _gson.fromJson(
				installDetailText, InstallDocuDetail.class);
		installDocuDetailNew.setCguid(_helper.generatePrimaryKey());

		int result = installDocuDetailMapper
				.insertSelective(installDocuDetailNew);
		return _helper.toJsonText(result, null);
	}
	...
	private InstallDocument createInstallDocumentFromEventInfo(
		...
		if (eventInfo != null) {
			try {
				...
				// 企业信息
				newInstallDocument.setEnterpriseBaseInfo(eventInfo
						.getEnterpriseBaseInfo());
				// 企业信息快照
				newInstallDocument.setCenterpriseid(eventInfo
						.getCenterpriseid());
				newInstallDocument.setCenterprisename(eventInfo
						.getCenterprisename());
				newInstallDocument.setCtaxcode(eventInfo.getCtaxcode());

				newInstallDocument.setCenterpriseadress(eventInfo
						.getCenterpriseadress());
				...
				newInstallDocument.setCcontactphone(eventInfo
						.getCcontactphone());
				newInstallDocument
						.setCarea(eventInfo.getAreaClass().getCname());
				...
				if (currentUser != null && currentUser.getEmployee() != null) {
					...
					if (currentUser.getEmployee().getDepartment() != null) {
						newInstallDocument.setCdepartment(currentUser
								.getEmployee().getDepartment().getCname());
					}
				...
				if (eventInfo.getEnterpriseBaseInfo() != null
					...
					newInstallDocument.setCbelongtax(eventInfo
							.getEnterpriseBaseInfo()
							.getCurrentTaxOrganization().getCguid());
				}
			...
			try {
				...
				if (installDetail.getMaterial() != null) {
					newInstallDocuDetail.setCname(installDetail.getMaterial()
							.getCname());
					newInstallDocuDetail.setCversion(installDetail
							.getMaterial().getCspec());
				}

				newInstallDocuDetail.setCrelationmatid(installDetail
						.getCmatid());
				newInstallDocuDetail.setRelateMaterial(installDetail
						.getMaterial());

				newInstallDocuDetail.setCinstalltypeid(installDetail
						.getCinstalltypeid());
				newInstallDocuDetail.setInstallType(installDetail
						.getInstallType());

				newInstallDocuDetail.setCismain(installDetail.getCismain());
				newInstallDocuDetail.setCisstatus(null);
				newInstallDocuDetail.setCchargetypeid(installDetail
						.getCchargetype());
				newInstallDocuDetail.setChargeType(installDetail
						.getChargeType());
				newInstallDocuDetail.setCresponseid(installDetail
						.getCresponseid());
				newInstallDocuDetail.setServiceResponse(installDetail
						.getServiceResponse());
				...
				if (currentInstallDocument != null) {
					newInstallDocuDetail.setCmainid(currentInstallDocument
							.getCguid());
					newInstallDocuDetail.setCcontactid(currentInstallDocument
							.getCcontactid());
					if (currentInstallDocument.getEnterpriseBaseInfo() != null) {
						newInstallDocuDetail
								.setContactInfo(currentInstallDocument
										.getEnterpriseBaseInfo()
										.getCurrentContact());
					}
				} else {
	...
	private void createExtendedAttrsForInstallDocDetail(
		...
		if (installDocuDetail.getRelateMaterial() != null) {
			List<ExtendedAttribute> extendedAttrs = installDocuDetail
					.getRelateMaterial().getExtendedAttributes();
			if (extendedAttrs != null && extendedAttrs.size() > 0) {
				...
				for (ExtendedAttribute extendedAttr : extendedAttrs) {
					...
					// 扩展属性可选值
					installDocuCofig.setEnumerateInfos(extendedAttr
							.getEnumerateInfos());
	...
	private void createDetailCode(InstallDocuDetail docuDetail,
		...
		if (StringUtils.isNotBlank(matId)
				&& (("247902721710531367".equals(matId)
						...
						|| ("132433004857549195").equals(matId) || ("132433004857549462")
							.equals(matId)))) {
			docuDetail.setCcode("");
		} else {
